...
if(isset($_POST['startdate'])){
    ...
    $type=$_POST['choice'];
    if($type=="date"){
         $queryNum = "SELECT COUNT(*) FROM orders WHERE $type BETWEEN '$sd' AND '$end_date'";
         $cho = "Collection Time";
    }else{
         $queryNum = "SELECT COUNT(*) FROM orders WHERE $type BETWEEN '$sd' AND '$end_date'";
         $cho = "Order Time";
    ...
    $pages->paginate();  
    if($num_rows[0]==0){
        $queryNum = "SELECT * FROM orders WHERE $type BETWEEN '$sd' AND '$end_date' ORDER BY $type DESC";
    }else{
        $queryNum = "SELECT * FROM orders WHERE $type BETWEEN '$sd' AND '$end_date' ORDER BY $type DESC $pages->limit ";
    }
...
$pages->mid_range = 9;  
$pages->paginate();  
$queryNum = "SELECT * FROM orders  ORDER BY order_id DESC $pages->limit ";
$resultO = mysqli_query($link, $queryNum) or die(mysqli_error($link));
}
...
?>                        Quick Filter:<input type='text' id='quickfind'/>
                ...
                Filter By: <select name="choice"><option value="date">Collection Time</option><option value="order_time">Order Time</option></select>
